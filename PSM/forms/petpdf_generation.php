...
if (isset($_GET['Pid'])) {
    $Pid = intval($_GET['Pid']);
    $query = "SELECT * FROM pet WHERE Pid = $Pid";
    ...
    if ($result && $result->num_rows > 0) {
        ...
        $pdf->Cell(0, 15, 'Pet Details Report', 0, 1, 'C');
        $pdf->Ln(5);

        // Table-like formatting
        $pdf->SetFont('Arial', '', 12);
        foreach ($row as $key => $value) {
            $label = ucfirst(str_replace(["Pid","Pname","Pbreed","Pspecies","Pgender","Oid"], 
                                         ["Pet ID","Pet Name","Breed","Species","Gender","Owner ID"], 
                                         $key));
