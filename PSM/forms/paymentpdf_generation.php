...
if (isset($_GET['Pay_id'])) {
    ...
    if ($result && $result->num_rows > 0) {
        ...
        $pdf->Cell(0, 15, 'Payment Details Report', 0, 1, 'C');
        $pdf->Ln(5);

        // Table-like formatting
        $pdf->SetFont('Arial', '', 12);
        foreach ($row as $key => $value) {
            $label = ucfirst(str_replace(["Pay_id","Aid","Pamount","Pmethode","Pstatus"], 
                                         ["Payment ID","Appointment ID","Payment Amount","Payment Methode","Payment Status"], 
                                         $key));
